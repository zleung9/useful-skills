...
Use the linked template at `templates/pmb-codex-lane-prompt.md` (in the kanban-worker skill) for prediction-market-bot work. For other repos, keep the same structure and replace the PMB-specific safety block with repo-specific invariants.
