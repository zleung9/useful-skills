...
```bash
cd /Users/zliang/Documents/nature-digest
```
...
skill** (msmtp, account `xmu`, sender `zliang8@xmu.edu.cn`) — already configured
...
- If relevant papers exist:
  ...
  ```
  The script sends via msmtp (`-a xmu`) if available; falls back to Resend if
...
- Email is sent via the **local email skill** (msmtp, account `xmu`). See
  `/Users/zliang/Nutstore Files/skills/email/SKILL.md` for details. Recipient:
  `zleung9@outlook.com`.
