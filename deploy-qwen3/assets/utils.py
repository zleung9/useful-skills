...
def enable_custom_op():
    ...
    try:
        ...
        # isort: on
        _CUSTOM_OP_ENABLED = False  # PATCHED(qwen6): CANN8.5.0 cannot load the 9.0.1-built custom aclnn ops (aclnnAddRmsNormBias etc. not in libopapi.so). Keep the vllm_ascend_C import above (it still registers torch.ops._C_ascend GDN ops) but disable the custom aclnn path so norms fall back to torch_npu built-ins (layernorm.py else-branches).
        ...
        if (not torch.compiler.is_compiling()) and "libcust_opapi.so" in str(e):
            try:
                ...
                _CUSTOM_OP_ENABLED = False  # PATCHED(qwen6): same as above.
            except ImportError:
                _CUSTOM_OP_ENABLED = False
                logger.warning(
